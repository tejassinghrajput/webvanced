...
export function HomeStats() {
  ...
  return (
    <section className="relative bg-gray-900 py-24 sm:py-32 text-white overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8 relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mx-auto max-w-2xl lg:mx-0">
          ...
          <p className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">{data.heading}</p>
          <div className="mt-6 text-lg text-gray-300 space-y-6">{data.paragraphs.map((p, i) => <p key={i}>{p}</p>)}</div>
        </motion.div>
        <dl className="mt-16 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {data.stats.map((stat, idx) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }} className="flex flex-col bg-white/5 p-8 rounded-2xl border border-white/10">
              ...
              <dd className="order-first text-4xl font-bold tracking-tight text-white"><Counter value={stat.value} /></dd>
            </motion.div>
          ))}
        </dl>
      </div>
